...
# M3: gcn_gater_pool_reg — GCN + Gater + HCGPool + Reg + Aux（全组件）
...
class GCN_Gater_Pool_Reg(nn.Module):
    """M3: GCN + Gater + HCGPool + Reg + Aux（全组件）"""
    def __init__(self, indim, nclass, num_clusters=8,
                 dropout=0.65, hidden=64, pool_temp=1.0,
                 llm_hidden=96, llm_layers=1, llm_heads=2, llm_dropout=0.5):
        super().__init__()
        ...
        self.aux_classifier = nn.Linear(num_clusters, nclass)
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        # 6. 计算每个图的聚类分布 p，用于 aux_classifier
        B_val = int(batch.max().item()) + 1 if batch.numel() > 0 else 1
        counts = torch.bincount(batch, minlength=B_val).to(x_conv.device)
        Nmax_val = int(counts.max().item()) if counts.numel() > 0 else 0
        s_b2 = s_matrix.new_zeros((B_val, Nmax_val, self.pool1.num_clusters))
        ptr2 = 0
        for i in range(B_val):
            ni = int(counts[i].item())
            if ni > 0:
                s_b2[i, :ni] = s_matrix[ptr2:ptr2 + ni]
                ptr2 += ni
        counts_f = counts.float().unsqueeze(1).clamp(min=EPS)
        p_cluster = s_b2.sum(dim=1) / counts_f
        aux_logits = self.aux_classifier(p_cluster)

        return F.log_softmax(self.fc2(x_out), dim=-1), s_matrix, x_conv, gate, aux_logits
        # 返回: output, s_matrix, x_conv, gate, aux_logits
...
def parse_args():
    parser = argparse.ArgumentParser(description='DS000030 Graph Classification Ablation')
    ...
    parser.add_argument('--save_path', type=str,
                        default='./model_ds30_ablation_randomsplit/')
...
def build_model(opt):
    ...
    M2 gcn_pool_reg       — GCN + HCGPool + Reg（无 Gater）
    M3 gcn_gater_pool_reg — GCN + Gater + HCGPool + Reg + Aux
    """
...
def run_single_experiment(run_idx, base_seed, dataset, opt):
    ...
    # ---------- 计算训练集类别权重（DS000030 样本量小，类别可能不均衡）----------
    def get_class_weights(loader):
        counts = torch.zeros(opt.nclass, device=device)
        for d in loader:
            for c in range(opt.nclass):
                counts[c] += (d.y == c).sum().item()
        w = counts.sum() / (opt.nclass * counts.clamp(min=1))
        return w
    ...
    # ---------- 统一前向，返回 (output, s_matrix, x_conv, gate, aux_logits) ----------
    def forward_model(x, edge_index, batch, edge_attr, pos):
        ...
        if mt == 'baseline':
            ...
            return output, None, None, gate, None
        elif mt == 'gcn_pool_reg':
            output, s_matrix, x_conv = model(x, edge_index, batch, edge_attr, pos)
            return output, s_matrix, x_conv, None, None
        elif mt == 'gcn_gater_pool_reg':
            output, s_matrix, x_conv, gate, aux = model(x, edge_index, batch, edge_attr, pos)
            return output, s_matrix, x_conv, gate, aux
        else:
            raise ValueError(f"Unknown model_type: {mt}")

    # ---------- 统一 loss 计算 ----------
    def compute_loss(output, s_matrix, x_conv, gate, aux_logits,
                     edge_index_orig, batch_orig, y):
        mt = opt.model_type
        ...
        if mt == 'baseline':
            ...
            loss_gate = (gate ** 2).mean() * 0.0007
            loss_reg = compute_reg_loss(s_matrix, x_conv, edge_index_orig, batch_orig)
            loss_aux = F.cross_entropy(aux_logits, y) if aux_logits is not None else 0.
    ...
    def train_one_epoch(epoch):
        ...
        for data in train_loader:
            ...
            output, s_matrix, x_conv, gate, aux_logits = forward_model(
                x_aug, ei_aug, data.batch, ea_aug, data.pos)
            # reg loss 使用原始 edge_index（未 dropout）
            loss = compute_loss(output, s_matrix, x_conv, gate, aux_logits,
...
def main():
    ...
    ch.setFormatter(fmt)
    fh = logging.FileHandler('ds30_ablation_randomsplit_10runs_stats.log', encoding='utf-8')
