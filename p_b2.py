...
class GCN_LLM(nn.Module):
    """M1: GCN + LLM Feature Gater (无 HCGPool)"""
    def __init__(self, indim, nclass, hidden=64, dropout=0.65,
...
class NetworkLLM_GNN(nn.Module):
    """
    通用模型类，用于:
    M2: GCN + LLM + HCGPool
    M3: GCN + LLM + HCGPool + Reg
    M4: GCN + LLM + HCGPool + Reg + Aux (Full)
    """
    def __init__(self, indim, nclass, num_clusters=8,
        ...
        if use_aux:
            self.aux_classifier = nn.Linear(self.num_clusters, nclass)
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        if batch.numel() == 0:
            ...
            B = int(batch.max().item()) + 1
        counts = torch.bincount(batch, minlength=B).to(x.device)
        Nmax = int(counts.max().item()) if counts.numel() > 0 else 0
        s_batched = s_matrix.new_zeros((B, Nmax, self.num_clusters))
        ptr = 0
        for i in range(B):
            ni = int(counts[i].item())
            if ni > 0:
                s_batched[i, :ni, :] = s_matrix[ptr:ptr + ni]
                ptr += ni
        counts_clamped = counts.clone().float().unsqueeze(1).clamp(min=EPS)
        p = s_batched.sum(dim=1) / counts_clamped

        if self.use_aux:
            aux_logits = self.aux_classifier(p)
        else:
            aux_logits = torch.zeros((B, output.size(-1)), device=x.device)

        return output, s_matrix, x_conv, aux_logits, p, gate
...
def cluster_reg_loss(x_conv, s_matrix, edge_index, batch, num_clusters, spec_k):
    ...
    loss_link = torch.mean(torch.sum(A_tilde_norm, dim=(1, 2)) - 2 * torch.einsum('bii->b', A_tilde_norm))
...
def parse_args():
    ...
    parser.add_argument('--batchSize', type=int, default=137)
    ...
    parser.add_argument('--lamb_aux', type=float, default=0.05)
    parser.add_argument('--spec_k', type=int, default=3)

    parser.add_argument('--indim', type=int, default=137)
    parser.add_argument('--nroi', type=int, default=137)
    parser.add_argument('--nclass', type=int, default=2)
    parser.add_argument('--load_model', type=bool, default=False)
    parser.add_argument('--save_model', type=bool, default=True)
    parser.add_argument('--optim', type=str, default='Adam')
    parser.add_argument('--save_path', type=str, default='./model_llm_hcg_ablation_randomsplit/')
    ...
    # 更改了choices以反映新的消融顺序
    parser.add_argument('--model_type', type=str, default='gcn_llm_pool',
                        choices=['baseline', 'gcn_llm', 'gcn_llm_pool', 'gcn_llm_pool_reg', 'full'])
    
    parser.add_argument('--val_size', type=float, default=0.2)
    parser.add_argument('--test_size', type=float, default=0.2)
    parser.add_argument('--num_runs', type=int, default=10,
                        help='重复运行次数用于统计均值/方差和 p 值')
...
def build_model(opt):
    ...
    根据新的消融顺序构建模型:
    1. baseline: 纯GCN
    2. gcn_llm: GCN + LLM Gater
    3. gcn_llm_pool: GCN + LLM Gater + HCGPool
    4. gcn_llm_pool_reg: GCN + LLM Gater + HCGPool + Reg
    """
    if opt.model_type == 'baseline':
        model = BaselineGCN(opt.indim, opt.nclass).to(device)
    elif opt.model_type == 'gcn_llm':
        model = GCN_LLM(opt.indim, opt.nclass).to(device)
    elif opt.model_type in ['gcn_llm_pool', 'gcn_llm_pool_reg', 'full']:
        use_aux = (opt.model_type == 'full')
...
def run_single_experiment(run_idx, base_seed, dataset, opt):
    ...
    def train_one_epoch(epoch):
        ...
        for data in train_loader:
            ...
            if opt.model_type == 'baseline':
                output = model(x_aug, ei_aug, data.batch, ea_aug, data.pos)
                loss_c = F.nll_loss(output, data.y)
                loss = loss_c
            elif opt.model_type == 'gcn_llm':
                output, gate = model(x_aug, ei_aug, data.batch, ea_aug, data.pos)
                loss_c = F.nll_loss(output, data.y)
                # LLM/TFG 引入的门控正则损失
                loss_gate = (gate ** 2).mean() * 0.0007
                loss = loss_c + loss_gate
            else:
                # 针对 gcn_llm_pool, gcn_llm_pool_reg, full
                output, s_matrix, x_conv, aux_logits, p, gate = model(
                ...
                # 始终计算 Cluster Reg Loss (哪怕不用，先算出数值)
                losses_dict, _, _ = cluster_reg_loss(
                    x_conv, s_matrix, data.edge_index, data.batch,
                    num_clusters=opt.num_clusters, spec_k=opt.spec_k)
                loss_cluster_base = losses_dict['loss_link'] + 0.5 * losses_dict['entropy']
                loss_orth = losses_dict['loss_orth']
                loss_balance = losses_dict['loss_balance']
                loss_spec = losses_dict['loss_spec']

                if opt.model_type == 'full': # 即包含 Aux
                    loss_aux = F.cross_entropy(aux_logits, data.y)
                else:
                    loss_aux = torch.tensor(0.0, device=device)

                if opt.model_type == 'gcn_llm_pool':
                    # GCN + LLM + Pool (无Reg, 无Aux)
                    loss = opt.lamb0 * loss_c + loss_gate
                elif opt.model_type == 'gcn_llm_pool_reg':
                    # GCN + LLM + Pool + Reg (无Aux)
                    loss = (opt.lamb0 * loss_c +
                            opt.lamb_cluster * loss_cluster_base +
                            0.015 * loss_orth +
                            0.002 * loss_balance +
                            opt.lamb_spec * loss_spec +
                            loss_gate)
                elif opt.model_type == 'full':
                    # All components
                    loss = (opt.lamb0 * loss_c +
                            opt.lamb_cluster * loss_cluster_base +
                            0.015 * loss_orth +
                            0.002 * loss_balance +
                            opt.lamb_spec * loss_spec +
                            opt.lamb_aux * loss_aux +
                            loss_gate)
                else:
                    raise ValueError(f"Unknown model type in training loop: {opt.model_type}")
    ...
    def eval_acc(loader):
        ...
        for data in loader:
            ...
            if opt.model_type == 'baseline':
                ...
                outputs, _ = model(data.x, data.edge_index, data.batch, data.edge_attr, data.pos)
                pred = outputs.max(dim=1)[1]
            else:
    ...
    def eval_loss(loader):
        ...
        for data in loader:
            data = data.to(device)
            if opt.model_type == 'baseline':
                output = model(data.x, data.edge_index, data.batch, data.edge_attr, data.pos)
                loss_c = F.nll_loss(output, data.y)
                loss = loss_c
            elif opt.model_type == 'gcn_llm':
                output, gate = model(data.x, data.edge_index, data.batch, data.edge_attr, data.pos)
                loss_c = F.nll_loss(output, data.y)
                loss_gate = (gate ** 2).mean() * 0.0007
                loss = loss_c + loss_gate
            else:
    ...
    for data in test_loader:
        ...
        all_probs_list.append(prob.cpu().detach().numpy())
        all_preds_list.append(pred.cpu().detach().numpy())
        all_trues_list.append(data.y.cpu().detach().numpy())
...
def main():
    ...
    ch.setFormatter(fmt)
    fh = logging.FileHandler('llm_full_randomsplit_10runs_stats.log', encoding='utf-8')
